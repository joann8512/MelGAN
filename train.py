...
def parse_args():
    ...
    parser.add_argument("--n_mel_channels", type=int, default=128)  # 80, !!!128
    ...
    parser.add_argument("--data_path", default=None, type=Path)  # type=str
...
def main():
    ...
    # Load PyTorch Models #
    #######################
    netG = Generator(args.n_mel_channels, args.ngf, args.n_residual_layers).cuda()  # 128, 32, 3
    ...
    # Create data loaders #
    #######################
    train_set = AudioDataset(Path(args.data_path) / "train_files_fix3.txt", args.seq_len, sampling_rate=16000) #22050, args.seq_len
    test_set = AudioDataset(Path(args.data_path) / "test_files_fix3.txt", 16000 * 4, sampling_rate=16000, augment=False)  # !!! 16000*2, don't change
    #train_set = AudioDataset("train_files_fix2.txt", args.seq_len, sampling_rate=16000) #22050
    #test_set = AudioDataset("test_files_fix2.txt", 16000 * 4, sampling_rate=16000, augment=False)

    train_loader = DataLoader(train_set, batch_size=args.batch_size, num_workers=4)  # batch=16
